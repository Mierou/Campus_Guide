      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--cream)', overflow: 'hidden' }}>
        ...
        <div className="page-header" style={{ justifyContent: 'space-between' }}>
          <div>
            ...
            <p style={{ fontSize: 12.5, color: 'var(--muted)', marginTop: 1 }}>Click a marker to view details</p>
          </div>
        ...
        <div style={{ flex: 1, position: 'relative', padding: 16, overflow: 'hidden' }}>
          {loading
            ? <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted)', fontSize: 14 }}>Loading campus data…</div>
            : <CampusMap markers={markers} height="calc(100vh - 90px)" flyTo={flyTo} onMarkerClick={handleMarkerClick} />
          }

          {/* Floating detail card */}
          {selected && (
            <div style={{ position: 'absolute', bottom: 32, left: 32, width: 300, background: 'var(--surface)', borderRadius: 16, border: '1px solid var(--border)', boxShadow: '0 8px 32px rgba(109,26,26,0.14)', overflow: 'hidden', zIndex: 1000 }} className="fade-up">
