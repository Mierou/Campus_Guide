...
import { Navigation, ArrowLeftRight, ChevronDown, ChevronUp } from 'lucide-react'
...
function RoutesInner() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      <Sidebar />
      <BottomNav />
      <main style={{ flex: 1, display: 'flex', overflow: 'hidden', background: 'var(--cream)' }} className="desktop-two-col">
        {/* Panel */}
        <div className="route-panel" style={{ width: 300, background: 'var(--surface)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', flexShrink: 0, overflowY: 'auto' }}>
          ...
          <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
            {locations.length === 0 ? (
              ...
              <>
                ...
                {/* Mobile map toggle */}
                <button onClick={() => setShowMap(v => !v)} className="btn-ghost" style={{ justifyContent: 'center', fontSize: 12.5, display: 'none' }} id="map-toggle">
                  <style>{`@media(max-width:768px){#map-toggle{display:flex!important}}`}</style>
                ...
                {showMap && (
                  <div id="mobile-map-r" style={{ display: 'none' }}>
                    ...
                    <CampusMap markers={markers} route={route} height="220px" />
                  </div>
                )}
                ...
                {noRoute && (
                  <div style={{ background: 'var(--amber-pale)', borderRadius: 12, border: '1px solid #f0d090', padding: 14, fontSize: 13, color: 'var(--amber)' }}>
                    <strong>No route available</strong> for this pair yet.
                  </div>
                )}
              </>
            )}
